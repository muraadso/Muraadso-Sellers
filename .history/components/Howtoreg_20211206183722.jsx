...
function createData(nooca, tusaale, fat) {
  return { nooca, tusaale, fat };
}
  ...
  return (
    <TableContainer className="" component={Paper} sx={{ padding: "12px"}}>
      <Table sx={{ minWidth: 700 }} aria-label="customized table">
        ...
        <TableBody>
          {rows.map((row) => (
            <StyledTableRow key={row.nooca} sx={{backgroundColor: 'white'}}>
              <StyledTableCell component="th" scope="row">
                {row.nooca}
              </StyledTableCell>
              <StyledTableCell  sx={{backgroundColor:'red'}} align="center">{row.tusaale}</StyledTableCell>
